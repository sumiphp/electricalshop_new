...
alert(ordid);
var billingdetails = function(ordid) {
    var getBrandTable = function() {
        var checkEL = setInterval(() => {
            if ($('.dataTable-carousal').length > 0) {
                clearInterval(checkEL);
                $.ajax({
                    type: 'POST',
                    //url: CustomUtils.siteUrl() + 'acp/ProductBrands/getBrandTable',
                    url: CustomUtils.siteUrl() + 'acp/Settings/getorderdetailsTable?ordid='+ordid,
                    success: function(res) {
                        $('.dataTable-carousal').html(res);
    ...
    var actions = function() {
        ...
        function doAction(action, brand) {
            $.ajax({
                type: 'POST',
                //url: CustomUtils.siteUrl() + 'acp/ProductBrands/action',
                url: CustomUtils.siteUrl() + 'acp/Settings/actionbill',
    ...
    var dataValidator = function() {
        ...
        $('body').on('click', '#saveBrandSubmit', function(ev) {
            ...
            if (error) {
                ...
                $.ajax({
                    type: 'POST',
                    url: CustomUtils.siteUrl() + 'acp/ProductBrands/save',
